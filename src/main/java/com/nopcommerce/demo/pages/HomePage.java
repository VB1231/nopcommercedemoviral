...
public class HomePage extends Utility {  private static final Logger log = LogManager.getLogger(HomePage.class.getName());
    ...
    public boolean isLogInLinkDisplay() {
        ...
        log.info("check login link displayed" +loginLink.toString());
    ...
    public boolean isLogOutLinkDisplay() {
        ...
        log.info("check is log out link displayed" +logOutLink.toString());
    ...
    public void clickOnMenuTab(String tab) {
        for (WebElement menu : topMenuList) {
            if (menu.getText().contains(tab)) {
                log.info("click on " +tab.toString());
